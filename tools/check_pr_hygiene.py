...
Both are easy to hit and neither is obvious after the fact:
...
def _auth_header() -> dict[str, str]:
    ...
    Deliberately not `git credential fill`: the credential helper stores a
    token for github.com (git over HTTPS), not for api.github.com, so it is
    the wrong lookup and would miss.
    """
    token = (
