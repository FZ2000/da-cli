...
#
# Rulesets (and the older branch-protection API) are refused on a free-plan
# PRIVATE repository:
#
#   403  Upgrade to GitHub Pro or make this repository public to enable
#        this feature.
#
# They are free once the repo is public. So this cannot be applied at setup
# time; it has to be run at the moment of publication. Until then the only
# guard against a direct push to main is .githooks/pre-push, which is
# client-side and bypassable — it stops accidents, not intent.
#
...
echo "merge into a broken main with no textual conflict — which is exactly"
echo "what happened on the previous forge."
