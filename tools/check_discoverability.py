...
# positive. What needs catching is a host only the author can resolve: a
# Tailscale magic-DNS name, a self-hosted service on a well-known local
# port, or an mDNS `.local` name.
PRIVATE_HOST = re.compile(
    # Requiring a subdomain in front lets prose *about* these patterns
    # mention `*.ts.net` without tripping the rule.
    r"[\w-]+\.ts\.net"  # Tailscale magic-DNS names
    r"|localhost:3000"  # common self-hosted forge port
    r"|127\.0\.0\.1:3000"
